...
function geodir_params() {
	...
	$default_marker_icon   = geodirectory()->maps->default_marker_icon( true );
	$default_marker_size   = geodirectory()->maps->get_marker_size( $default_marker_icon, array( 'w' => 20, 'h' => 34 ) );
	...
	$arr_alert_msg = array(
		...
		'map_name'                                     => geodirectory()->maps->active_map(),
		...
		'google_api_key'                               => geodirectory()->maps->google_api_key(),
		'mapLanguage'                                  => geodirectory()->maps->map_language(),
		'osmRouteLanguage'                             => geodirectory()->maps->osm_routing_language(),
...
//	add_action( 'wp_print_footer_scripts', array( 'GeoDir_Maps', 'check_map_script' ), 99999 ); //@todo do we need to add a call here?
...
function geodir_search_form() {
	...
	if ( wp_doing_ajax() && ! empty( $_POST['keepArgs'] ) ) {
		...
		$KeepArgs = str_replace( array( '“', '”', '″', '„', '« ', ' »' ), '"', trim( $_POST['keepArgs'] ) );
