...
/**
 * Renders the add listing map and enqueues the necessary scripts and data.
 *
 * @global int    $mapzoom             Zoom level value for the map.
 * @global bool   $geodir_manual_map   Check if manual map.
 * @global string $prefix              The prefix for form field IDs.
 * @global string $lat                 The latitude for the map center.
 * @global string $lng                 The longitude for the map center.
 * @global string $city                The city name for map restriction.
 * @global string $map_title           The title for the "Set Address on Map" button.
 * @global string $design_style        The current design style (e.g., 'bootstrap').
 */
function geodir_display_add_listing_map_view($prefix, $map_title, $country, $region, $city, $lat, $lng ) {
...
//	echo '###'.$prefix;
	...
	// ### 1. GATHER ALL PHP DATA ###
	// =========================================================================

	$horizontal = empty( $geodir_label_type ) || $geodir_label_type == 'horizontal';
	...
	$default_lat = isset( $default_location->latitude ) ? $default_location->latitude : '39.952484';
	$default_lng = isset( $default_location->longitude ) ? $default_location->longitude : '-75.163786';
	$default_city = isset( $default_location->city ) ? $default_location->city : '';
	$default_region = isset( $default_location->region ) ? $default_location->region : '';
	$default_country = isset( $default_location->country ) ? $default_location->country : '';

	$lat_lng_blank = ( empty( $lat ) && empty( $lng ) );
	...
	if ( is_admin() && isset( $_REQUEST['tab'] ) && empty( $mapzoom ) ) {
		$mapzoom = isset( $_REQUEST['add_hood'] ) ? 10 : 4;
	}
	$mapzoom = $mapzoom ? absint( $mapzoom ) : 12;

	$marker_icon = GeoDir_Maps::default_marker_icon( true );
	$icon_size = GeoDir_Maps::get_marker_size( $marker_icon, array( 'w' => 20, 'h' => 34 ) );

	$default_iso2 = '';
	if ( ! defined( 'GEODIRLOCATION_TEXTDOMAIN' ) && !empty($default_location) ) {
		$default_iso2 = $geodirectory->location->get_country_iso2( $default_location->country );
	}


	// ### 2. PREPARE DATA FOR JAVASCRIPT ###
	// =========================================================================
	// This array will be passed to the JavaScript file using wp_localize_script.

	$script_data = array(
		'prefix'      => $prefix,
		'lat'         => geodir_sanitize_float( $lat ),
		'lng'         => geodir_sanitize_float( $lng ),
		'default_lat' => geodir_sanitize_float( $default_lat ),
		'default_lng' => geodir_sanitize_float( $default_lng ),
		'mapzoom'     => $mapzoom,
		'lat_lng_blank' => $lat_lng_blank,
		'is_map_restrict' => $is_map_restrict,
		'minZoomLevel' => $is_map_restrict ? 5 : 0,
		'mapLang'     => esc_js( GeoDir_Maps::map_language() ),
		'auto_change_map_fields' => $auto_change_map_fields,
		'auto_change_address_fields_pin_move' => $auto_change_address_fields_pin_move,
		'city'        => addslashes_gpc( $city ),
		'region'      => addslashes_gpc( $default_region ),
		'country'     => addslashes_gpc( $default_country ),
		'default_iso2' => $default_iso2,
		'geocode_region_level' => json_decode( apply_filters( "geodir_geocode_region_level", '["GB","ES"]' ) ),
		'split_uk'    => geodir_split_uk(),
		'marker_icon' => $marker_icon,
		'icon_size_w' => $icon_size['w'],
		'icon_size_h' => $icon_size['h'],
		'resize_marker' => apply_filters( 'geodir_map_marker_resize_marker', false ),
		'manual_map'  => ! empty( $geodir_manual_map ),
		'lazy_load_map' => geodir_lazy_load_map(),
		'mouse_scroll' => geodir_get_option('geodir_add_listing_mouse_scroll'),
		'is_admin' => is_admin() && isset($_REQUEST['tab']),
		'i18n'        => array( // Internationalization strings
			'cannot_determine_address' => __( 'Cannot determine address at this location.', 'geodirectory' ),
			'choose_address_in_city'   => wp_sprintf( __( 'Please choose any address of the (%s) city only.', 'geodirectory' ), $city ),
			'geocode_fail'             => __( 'Geocode was not successful for the following reason:', 'geodirectory' ),
			'set_address_help'         => __( 'Click on "Set Address on Map" and then you can also drag map marker to locate the correct address', 'geodirectory' ),
		),
		// Pass any other necessary data here
	);

	// This is where you would enqueue the script and localize it.
	// Example (to be placed in your plugin's main file or functions.php):
	// wp_enqueue_script( 'geodir-add-listing-map', 'path/to/your/geodir-add-listing-map.js', array( 'jquery', 'geodir-go-map' ), '1.0', true );
	// wp_localize_script( 'geodir-add-listing-map', 'geodirMapData', $script_data );

	// For the template to work, we'll just add the script data directly for now.
	// In a real plugin, you should move this to a proper enqueue function.
	wp_add_inline_script( 'geodir-add-listing', 'var geodirMapData = ' . wp_json_encode( $script_data ) . ';', 'before' );
	...
	<?php if ( ! wp_doing_ajax() ) { ?>
		<div class="<?php echo ( $aui_bs5 ? 'mb-3' : 'form-group' ); ?><?php echo ( $horizontal ? ' row' : '' ); ?>">
			...
			<div class="col-sm-10">
				...
				<?php
				aui();
				echo AUI_Component_Helper::help_text( stripslashes( $script_data['i18n']['set_address_help'] ) );
				?>
				<?php if ( $horizontal ) { ?>
			</div>
		<?php } ?>
	...
	<div class="<?php echo ( $aui_bs5 ? 'mb-3' : 'form-group' ); ?> d-block">
		...
		$_template = 'bootstrap/map/map.php';
...
//
//
//function my_geodirectory_enqueue_map_script() {
//
//	// This condition ensures the script only loads on the add/edit listing pages,
//	// or wherever the 'geodir-add-listing' script is already being loaded.
//	if ( wp_script_is( 'geodir-add-listing', 'enqueued' ) ) {
//
//		// Register your new script
//		wp_enqueue_script(
//			'geodir-add-listing-map', // A unique handle for your script
//			geodir_plugin_url() . '/assets/js/geodir-add-listing-map.js', // The path to your new JS file
//			array( 'jquery', 'geodir-add-listing' ), // Dependencies: it needs jQuery and the main GD script
//			'1.0.0', // Your script's version number
//			true // Load the script in the footer
//		);
//	}
//}
//// Hook into WordPress's script-loading process
//add_action( 'wp_enqueue_scripts', 'my_geodirectory_enqueue_map_script' );
//add_action( 'admin_enqueue_scripts', 'my_geodirectory_enqueue_map_script' );


// Make sure you have a function like this hooked to 'wp_enqueue_scripts' and 'admin_enqueue_scripts'

function my_geodirectory_enqueue_map_scripts() {
	// This condition ensures scripts only load where GeoDirectory's add/edit form is present.
	if ( wp_script_is( 'geodir-add-listing', 'enqueued' ) ) {

		// 1. Enqueue the NEW Core Handler FIRST
		// It depends on jQuery and the goMap library.
		wp_enqueue_script(
			'geodir-map-handler', // A new, unique handle for the core handler class
			geodir_plugin_url() . '/assets/js/geodir-map-handler.js',
			array('jquery'),
			'3.0.0', // New version
			true     // Load in footer
		);

		// 2. Enqueue the Legacy Shim SECOND
		// It now depends on our new handler.
		wp_enqueue_script(
			'geodir-add-listing-map', // Use the original handle to replace the old script
			geodir_plugin_url() . '/assets/js/geodir-add-listing-map.js',
			array('geodir-map-handler'), // **CRITICAL**: This declares the dependency
			'3.0.0',
			true
		);
	}
...
if ( ! empty( $gd_move_inline_script ) ) { ob_start(); } else { ?>
	<script>
		...
		function updateMarkerAddress(getAddress, getZip, getCity, getState, getCountry, getAddress2, baseCountry) {
			console.log('updateMarkerAddress', getAddress, getZip, getCity, getState, getCountry, getAddress2, baseCountry);
	...
	<script>
		...
		window.geodir_listing_location_map = function(field) {
			// You can use the properties from the field object (field.id, field.label, etc.)
			// to build your custom HTML.

			// The returned HTML should include the necessary x-model and @change directives
			// to integrate with your Alpine.js data store.
