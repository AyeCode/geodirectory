...
class GeoDir_REST_Authentication {
	...
	private function perform_basic_authentication() {
		...
		if ( ! hash_equals( $this->user->consumer_secret, $consumer_secret ) ) {
			$this->set_error( new WP_Error( 'geodir_rest_authentication_error', __( 'Consumer secret is invalid.', 'geodirectory' ), array( 'status' => 401 ) ) );
	...
	public function get_oauth_parameters() {
		...
		if ( ! empty( $errors ) ) {
			$message = sprintf(
				_n( 'Missing OAuth parameter %s', 'Missing OAuth parameters %s', count( $errors ), 'geodirectory' ),
				implode( ', ', $errors )
			);

			$this->set_error( new WP_Error( 'geodir_rest_authentication_missing_parameter', $message, array( 'status' => 401 ) ) );
	...
	private function perform_oauth_authentication() {
		...
		if ( empty( $this->user ) ) {
			$this->set_error( new WP_Error( 'geodir_rest_authentication_error', __( 'Consumer key is invalid.', 'geodirectory' ), array( 'status' => 401 ) ) );
	...
	private function check_oauth_signature( $user, $params ) {
		...
		if ( ! uksort( $params, 'strcmp' ) ) {
			return new WP_Error( 'geodir_rest_authentication_error', __( 'Invalid signature - failed to sort parameters.', 'geodirectory' ), array( 'status' => 401 ) );
		}
		...
		if ( 'HMAC-SHA1' !== $params['oauth_signature_method'] && 'HMAC-SHA256' !== $params['oauth_signature_method'] ) {
			return new WP_Error( 'geodir_rest_authentication_error', __( 'Invalid signature - signature method is invalid.', 'geodirectory' ), array( 'status' => 401 ) );
		}
		...
		if ( ! hash_equals( $signature, $consumer_signature ) ) {
			return new WP_Error( 'geodir_rest_authentication_error', __( 'Invalid signature - provided signature does not match.', 'geodirectory' ), array( 'status' => 401 ) );
		}
	...
	private function join_with_equals_sign( $params, $query_params = array(), $key = '' ) {
		foreach ( $params as $param_key => $param_value ) {
			...
			if ( is_array( $param_value ) ) {
				...
				$query_params[] = geodir_rest_urlencode_rfc3986( $string );
			}
		}
	...
	private function normalize_parameters( $parameters ) {
		$keys       = geodir_rest_urlencode_rfc3986( array_keys( $parameters ) );
		$values     = geodir_rest_urlencode_rfc3986( array_values( $parameters ) );
	...
	private function check_oauth_timestamp_and_nonce( $user, $timestamp, $nonce ) {
		...
		if ( ( $timestamp < time() - $valid_window ) || ( $timestamp > time() + $valid_window ) ) {
			return new WP_Error( 'geodir_rest_authentication_error', __( 'Invalid timestamp.', 'geodirectory' ), array( 'status' => 401 ) );
		}
		...
		if ( in_array( $nonce, $used_nonces ) ) {
			return new WP_Error( 'geodir_rest_authentication_error', __( 'Invalid nonce - nonce has already been used.', 'geodirectory' ), array( 'status' => 401 ) );
		}
	...
	private function get_user_data_by_consumer_key( $consumer_key ) {
		...
		$consumer_key = geodir_api_hash( sanitize_text_field( $consumer_key ) );
		...
		switch ( $method ) {
			...
			case 'GET' :
				if ( 'read' !== $permissions && 'read_write' !== $permissions ) {
					return new WP_Error( 'geodir_rest_authentication_error', __( 'The API key provided does not have read permissions.', 'geodirectory' ), array( 'status' => 401 ) );
				}
				break;
			case 'POST' :
			case 'PUT' :
			...
			case 'DELETE' :
				if ( 'write' !== $permissions && 'read_write' !== $permissions ) {
					return new WP_Error( 'geodir_rest_authentication_error', __( 'The API key provided does not have write permissions.', 'geodirectory' ), array( 'status' => 401 ) );
				}
				break;
			case 'OPTIONS' :
				return true;
			...
			default :
				return new WP_Error( 'geodir_rest_authentication_error', __( 'Unknown request method.', 'geodirectory' ), array( 'status' => 401 ) );
		}
	...
	public function send_unauthorized_headers( $response ) {
		if ( is_wp_error( $this->get_error() ) && 'basic_auth' === $this->auth_method ) {
			$auth_message = __( 'GeoDirectory API. Use a consumer key in the username field and a consumer secret in the password field.', 'geodirectory' );
...
new GeoDir_REST_Authentication();
