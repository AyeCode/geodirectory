 ...
 */
class GeoDir_REST_Setting_Options_Controller extends GeoDir_REST_Controller {
	...
	public function register_routes() {
		register_rest_route( $this->namespace, '/' . $this->rest_base, array(
			'args' => array(
				'group' => array(
					'description' => __( 'Settings group ID.', 'geodirectory' ),
					'type'        => 'string',
				),
			),
			array(
	...
	public function get_group_settings( $group_id ) {
		if ( empty( $group_id ) ) {
			return new WP_Error( 'rest_setting_setting_group_invalid', __( 'Invalid setting group.', 'geodirectory' ), array( 'status' => 404 ) );
		}

		$settings = apply_filters( 'geodir_settings-' . $group_id, array() );
		...
		foreach ( $settings as $setting ) {
			...
			if ( is_array( $option_key ) ) {
				...
				$admin_setting_value = GeoDir_Admin_Settings::get_option( $option_key, $default );
				$setting['value']    = $admin_setting_value;
			}

			if ( 'multi_select_countries' === $setting['type'] ) {
				$setting['options'] = geodir_get_countries();
	...
	public function get_setting( $group_id, $setting_id ) {
		if ( empty( $setting_id ) ) {
			return new WP_Error( 'rest_setting_setting_invalid', __( 'Invalid setting.', 'geodirectory' ), array( 'status' => 404 ) );
		}
	...
	public function update_item( $request ) {
		...
		if ( is_array( $setting['option_key'] ) ) {
			...
			$setting['value']                      = $value;
			GeoDir_Admin_Settings::save_fields( array( $setting ), $update_data );
		}
	...
	public function get_items_permissions_check( $request ) {
		if ( ! geodir_rest_check_manager_permissions( 'settings', 'read' ) ) {
	...
	public function update_items_permissions_check( $request ) {
		if ( ! geodir_rest_check_manager_permissions( 'settings', 'edit' ) ) {
			return new WP_Error( 'geodir_rest_cannot_edit', __( 'Sorry, you cannot edit this resource.', 'geodirectory' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties'           => array(
				'id'               => array(
					'description'  => __( 'A unique identifier for the setting.', 'geodirectory' ),
					'type'         => 'string',
					'arg_options'  => array(
				...
				'label'            => array(
					'description'  => __( 'A human readable label for the setting used in interfaces.', 'geodirectory' ),
					'type'         => 'string',
					'arg_options'  => array(
				...
				'description'      => array(
					'description'  => __( 'A human readable description for the setting used in interfaces.', 'geodirectory' ),
					'type'         => 'string',
					'arg_options'  => array(
						'sanitize_callback' => 'sanitize_text_field',
					),
				...
				'value'          => array(
					'description'  => __( 'Setting value.', 'geodirectory' ),
					'type'         => 'mixed',
					'context'      => array( 'view', 'edit' ),
				),
				'default'          => array(
					'description'  => __( 'Default value for the setting.', 'geodirectory' ),
					'type'         => 'mixed',
					'context'      => array( 'view', 'edit' ),
					'readonly'     => true,
				),
				'tip'              => array(
					'description'  => __( 'Additional help text shown to the user about the setting.', 'geodirectory' ),
					'type'         => 'string',
					'arg_options'  => array(
						'sanitize_callback' => 'sanitize_text_field',
					),
				...
				'placeholder'      => array(
					'description'  => __( 'Placeholder text to be displayed in text inputs.', 'geodirectory' ),
					'type'         => 'string',
					'arg_options'  => array(
						'sanitize_callback' => 'sanitize_text_field',
					),
				...
				'type'             => array(
					'description'  => __( 'Type of setting.', 'geodirectory' ),
					'type'         => 'string',
					'arg_options'  => array(
						'sanitize_callback' => 'sanitize_text_field',
					),
				...
				'options'          => array(
					'description' => __( 'Array of options (key value pairs) for inputs such as select, multiselect, and radio buttons.', 'geodirectory' ),
					'type'        => 'object',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
