 ...
 */
class GeoDir_REST_Settings_Controller extends GeoDir_REST_Controller {
	...
	public function get_items( $request ) {
		$groups = apply_filters( 'geodir_settings_groups', array() );
		if ( empty( $groups ) ) {
			return new WP_Error( 'rest_setting_groups_empty', __( 'No setting groups have been registered.', 'geodirectory' ), array( 'status' => 500 ) );
		}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				'id' => array(
					'description' => __( 'A unique identifier that can be used to link settings together.', 'geodirectory' ),
					'type'        => 'string',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
				...
				'parent_id' => array(
					'description' => __( 'ID of parent grouping.', 'geodirectory' ),
					'type'        => 'string',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
				'sub_groups' => array(
					'description' => __( 'IDs for settings sub groups.', 'geodirectory' ),
					'type'        => 'string',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
