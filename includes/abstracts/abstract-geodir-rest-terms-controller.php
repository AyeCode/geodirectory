 ...
 * @version  2.0.0
 */
abstract class GeoDir_REST_Terms_Controller extends GeoDir_REST_Controller {
		...
		register_rest_route( $this->namespace, '/' . $this->rest_base, array(
			...
			array(
				...
				'args'                => array_merge( $this->get_endpoint_args_for_item_schema( WP_REST_Server::CREATABLE ), array(
					'name' => array(
						'type'        => 'string',
						'description' => __( 'Name for the resource.', 'geodirectory' ),
						'required'    => true,
					),
				) ),
			),
			...
			array(
				...
				'args'                => array(
					'force' => array(
						'default'     => false,
						'type'        => 'boolean',
						'description' => __( 'Required to be true, as resource does not support trashing.', 'geodirectory' ),
					),
				),
			),
			'schema' => array( $this, 'get_public_item_schema' ),
	...
	public function create_item_permissions_check( $request ) {
		...
		if ( ! $permissions ) {
			return new WP_Error( 'geodir_rest_cannot_create', __( 'Sorry, you are not allowed to create resources.', 'geodirectory' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function update_item_permissions_check( $request ) {
		...
		if ( ! $permissions ) {
			return new WP_Error( 'geodir_rest_cannot_edit', __( 'Sorry, you are not allowed to edit this resource.', 'geodirectory' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function delete_item_permissions_check( $request ) {
		...
		if ( ! $permissions ) {
			return new WP_Error( 'geodir_rest_cannot_delete', __( 'Sorry, you are not allowed to delete this resource.', 'geodirectory' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function batch_items_permissions_check( $request ) {
		...
		if ( ! $permissions ) {
			return new WP_Error( 'geodir_rest_cannot_batch', __( 'Sorry, you are not allowed to batch manipulate this resource.', 'geodirectory' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	protected function check_permissions( $request, $context = 'read' ) {
		...
		if ( ! $taxonomy ) {
			return new WP_Error( 'geodir_rest_taxonomy_invalid', __( 'Taxonomy does not exist.', 'geodirectory' ), array( 'status' => 404 ) );
		}
		...
		if ( $id = intval( $request['id'] ) ) {
			...
			if ( ! $term || $term->taxonomy !== $taxonomy ) {
				return new WP_Error( 'geodir_rest_term_invalid', __( 'Resource does not exist.', 'geodirectory' ), array( 'status' => 404 ) );
			}

			return geodir_rest_check_post_term_permissions( $taxonomy, $context, $term->term_id );
		}

		return geodir_rest_check_post_term_permissions( $taxonomy, $context );
	}
	...
	public function get_items( $request ) {
		 ...
		 */
		$prepared_args = apply_filters( "geodir_rest_{$taxonomy}_query", $prepared_args, $request );

		if ( ! empty( $prepared_args['post'] )  ) {
			$query_result = $this->get_terms_for_post( $prepared_args, $request );
	...
	public function create_item( $request ) {
		...
		if ( isset( $request['parent'] ) ) {
			if ( ! is_taxonomy_hierarchical( $taxonomy ) ) {
				return new WP_Error( 'geodir_rest_taxonomy_not_hierarchical', __( 'Can not set resource parent, taxonomy is not hierarchical.', 'geodirectory' ), array( 'status' => 400 ) );
			}
			...
			if ( ! $parent || is_wp_error( $parent ) ) {
				return new WP_Error( 'geodir_rest_term_invalid', __( 'Parent resource does not exist.', 'geodirectory' ), array( 'status' => 404 ) );
			}
		 ...
		 */
		do_action( "geodir_rest_insert_{$taxonomy}", $term, $request, true );
	...
	public function update_item( $request ) {
		...
		if ( isset( $request['parent'] ) ) {
			...
			if ( 0 === $parent_id ) {
				...
				if ( ! $parent ) {
					return new WP_Error( 'geodir_rest_term_invalid', __( 'Parent resource does not exist.', 'geodirectory' ), array( 'status' => 400 ) );
				}
		 ...
		 * @param boolean         $creating  True when creating term, false when updating.
		 */
		do_action( "geodir_rest_insert_{$taxonomy}", $term, $request, false );
	...
	public function delete_item( $request ) {
		...
		if ( ! $force ) {
			return new WP_Error( 'geodir_rest_trash_not_supported', __( 'Resource does not support trashing.', 'geodirectory' ), array( 'status' => 501 ) );
		}
		...
		if ( ! $retval ) {
			return new WP_Error( 'geodir_rest_cannot_delete', __( 'The resource cannot be deleted.', 'geodirectory' ), array( 'status' => 500 ) );
		}
		 ...
		 */
		do_action( "geodir_rest_delete_{$taxonomy}", $term, $response, $request );
	 ...
	 */
	protected function get_terms_for_post( $prepared_args, $request ) {
		$taxonomy = $this->get_taxonomy( $request );

		$query_result = get_the_terms( $prepared_args['post'], $taxonomy );
	...
	public function get_collection_params() {
		...
		$params['exclude'] = array(
			'description'       => __( 'Ensure result set excludes specific ids.', 'geodirectory' ),
			'type'              => 'array',
			'items'             => array(
				'type'          => 'integer',
			),
		...
		$params['include'] = array(
			'description'       => __( 'Limit result set to specific ids.', 'geodirectory' ),
			'type'              => 'array',
			'items'             => array(
				'type'          => 'integer',
			),
		...
		if ( ! $taxonomy->hierarchical ) {
			$params['offset'] = array(
				'description'        => __( 'Offset the result set by a specific number of items.', 'geodirectory' ),
				'type'               => 'integer',
		...
		$params['orderby']    = array(
			'description'           => __( 'Sort collection by resource attribute.', 'geodirectory' ),
			'type'                  => 'string',
			'sanitize_callback'     => 'sanitize_key',
			'default'               => 'name',
			'enum'                  => array(
		...
		$params['hide_empty'] = array(
			'description'           => __( 'Whether to hide resources not assigned to any posts.', 'geodirectory' ),
			'type'                  => 'boolean',
			'default'               => false,
			'validate_callback'     => 'rest_validate_request_arg',
		);
		if ( $taxonomy->hierarchical ) {
			$params['parent'] = array(
				'description'        => __( 'Limit result set to resources assigned to a specific parent.', 'geodirectory' ),
				'type'               => 'integer',
				'sanitize_callback'  => 'absint',
				'validate_callback'  => 'rest_validate_request_arg',
			);
		...
		$params['post'] = array(
			'description'           => __( 'Limit result set to resources assigned to a specific post.', 'geodirectory' ),
			'type'                  => 'integer',
			'default'               => null,
			'validate_callback'     => 'rest_validate_request_arg',
		);
		$params['slug']    = array(
			'description'        => __( 'Limit result set to resources with a specific slug.', 'geodirectory' ),
			'type'               => 'string',
			'validate_callback'  => 'rest_validate_request_arg',
		);
