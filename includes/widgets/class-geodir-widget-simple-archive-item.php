...
class GeoDir_Widget_Simple_Archive_Item extends WP_Super_Duper {
	...
	public function get_badge_type( $type, $args = array(), $badge_args = array() ) {
		...
		if ( $type == 'featured' ) {
			$output = "[gd_post_badge key='featured' condition='is_not_empty' badge='". __( 'Featured', 'geodirectory' ) ."' bg_color='#fd4700' txt_color='#ffffff' css_class='' $alignment $position_args]";
		} elseif ( $type == 'new' ) {
			$output = "[gd_post_badge id=''  key='post_date'  condition='is_less_than'  search='+30'  icon_class=''  badge='". __( 'New', 'geodirectory' ) ."'  link=''  new_window='false'  popover_title=''  popover_text=''  cta=''  tooltip_text=''  hover_content=''  hover_icon=''  type=''  shadow=''  color=''  bg_color='#ff0000'  txt_color='#ffffff'  size=''  $alignment  mb=''  ml='' $position_args  list_hide=''  list_hide_secondary=''  css_class='' ]";
