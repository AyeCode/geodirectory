...
(function(root, factory) {
    ...
    L.TileLayer.Provider = L.TileLayer.extend({
        initialize: function(arg, options) {
            ...
            if (provider.url.indexOf("//") === 0 && forceHTTP) provider.url = "http:" + provider.url;
            if (provider.options.retina)
                if (options.detectRetina && L.Browser.retina) options.detectRetina = false;
                else provider.options.retina = "";
    ...
    if (typeof geodir_var != "undefined" && geodir_var.osmProviders && typeof geodir_var.osmProviders == "object") try {
        L.TileLayer.Provider.providers = geodir_var.osmProviders
    } catch (e) {}
...
var exampleAPIcodes = typeof geodir_var != "undefined" && geodir_var.osmAPIcodes ? geodir_var.osmAPIcodes : {};
...
L.TileLayer.Provider.include({
    initialize: function(providerName, options) {
        ...
        if (provider in exampleAPIcodes) {
            ...
            for (var key in exampleAPIcodes[provider]) this._exampleAPIcodes[key] = "<your " + key + ">";
...
L.tileLayer.provider.eachLayer = function(callback) {
    for (var provider in L.TileLayer.Provider.providers)
        if (L.TileLayer.Provider.providers[provider].variants)
            for (var variant in L.TileLayer.Provider.providers[provider].variants) callback(provider + "." + variant);
