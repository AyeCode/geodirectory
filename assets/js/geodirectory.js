...
jQuery.fn.gdunveil = function(threshold, callback, extra1) {
    var $w = jQuery(window),
        ...
        images = this,
        loaded;
    if (extra1) var $e1 = jQuery(extra1),
        th = threshold || 0,
        retina = window.devicePixelRatio > 1,
        attrib = retina ? "data-src-retina" : "data-src",
        images = this,
...
jQuery(function($) {
    ...
    $("body").bind("geodir_map_infowindow_open", function(e, data) {
        if (data.content && $(data.content).find(".gd-bh-show-field").length) geodir_refresh_business_hours();
...
jQuery(document).ready(function($) {
    ...
    jQuery(".geodir-delete").click(function() {
        ...
        if (jQuery(this).closest(".geodir-gridview").hasClass("gdp-franchise-m") || jQuery(this).closest(".geodir-listview").hasClass("gdp-franchise-m")) message = geodir_params.my_main_listing_del;
...
jQuery(window).load(function() {
    jQuery(".geodir-tabs-content").show();
    tabNoRun = false;

    function activateTab(tab) {
        if (!jQuery(".geodir-tab-head").length) return;
        tabName = urlHash = tab.find("a").html();
        if (tabName && jQuery(".geodir-mobile-active-tab").length) jQuery(".geodir-mobile-active-tab").html(tabName);
        if (tabNoRun) {
            tabNoRun = false;
            return
        }
        ...
        else if (typeof urlHash === "undefined") {
            if (window.location.hash.substring(0, 8) == "#comment") {
                tab = jQuery('*[data-tab="#reviews"]').parent();
                tabNoRun = true
            }
        } else if (history.pushState) history.replaceState(null, null, urlHash);
        else window.location.hash = urlHash;
        ...
        if (urlHash == "#post_map" && window.gdMaps) window.setTimeout(function() {
    ...
    jQuery("dl.geodir-tab-head").each(function() {
        ...
        tabs.click(function(e) {
            if (jQuery(this).find("a").attr("data-status") == "enable") activateTab(jQuery(this))
        })
    });
    if (window.location.hash) activateTab(jQuery('a[data-tab="' + window.location.hash + '"]').parent());
    jQuery(".gd-tabs .gd-tab-next").click(function(ele) {
        var is_validate = true;
        if (is_validate) {
            var tab = jQuery("dl.geodir-tab-head").find("dd.geodir-tab-active").next();
            if (tab.find("a").attr("data-status") == "enable") activateTab(tab);
...
jQuery(document).ready(function() {
    ...
    jQuery(".gd-trigger").click(function() {
        ...
        if (jQuery(".gd-trigger").hasClass("gd-triggeroff")) {
            ...
            if (aD) gd_compress_animate(this, 0)
        } else {
            jQuery(".gd-trigger").removeClass("gd-triggeron");
            jQuery(".gd-trigger").addClass("gd-triggeroff");
            if (aD) gd_compress_animate(this, parseFloat(jQuery(this).toggleClass("active").next().outerWidth()))
        }
    });
    jQuery(".gd-trigger").each(function() {
        if (jQuery(this).hasClass("gd-triggeroff") && jQuery(this).next().hasClass("map_category")) gd_compress_animate(this, parseFloat(jQuery(this).next().outerWidth()))
    });
    ...
    jQuery("#commentform .gd_rating, #commentform .gd-fa-rating").each(function() {
        ...
        if (!geodir_params.multirating) $frm_obj.find('input[name="submit"]').click(function(e) {
            $frm_obj.find("#err_no_rating").remove();
            if (typeof geodir_params.gd_cmt_disable_rating != "undefined" && geodir_params.gd_cmt_disable_rating) return true;
...
jQuery(window).load(function() {
    if (jQuery(".map_background").length == 0) jQuery(".geodir-pinpoint").hide();
    else jQuery(".geodir-pinpoint").show()
});
...
function geodir_get_post_term(el) {
    ...
    jQuery.post(geodir_params.ajax_url + "?action=geodir_ajax_action", {
        ...
        if (jQuery.trim(data) != "") {
            ...
            if (jQuery(el).parent().parent().find(".geodir-popular-cat-list li").length > limit) jQuery(el).parent().parent().parent().find(".geodir-cat-list-more").fadeIn("slow")
        }
    })
}
jQuery(window).load(function() {
    ...
    jQuery(document).on("click", ".geodir-rating,.gd-star-rating", function(e) {
        if (reviewLink = jQuery(this).closest(".geodir-category-listing").find("a.geodir-pcomments").attr("href")) window.location = reviewLink;
        else if (reviewLink = jQuery(this).closest(".gd-bubble").find("a.geodir-pcomments").attr("href")) window.location = reviewLink
    });
...
function geodir_setup_search_form() {
    if (jQuery(".search_by_post").val()) gd_s_post_type = jQuery(".search_by_post").val();
    else gd_s_post_type = "gd_place";
    setTimeout(function() {
...
function geodir_search_wait(on) {
    waitTime = 300;
    if (on) {
        var geodir_search_wait_animate = function() {
            if (!searchPos) return;
            if (searchPos == 1) {
...
function gd_fav_save(post_id) {
    var ajax_action;
    if (jQuery(".favorite_property_" + post_id + " a").hasClass("geodir-removetofav-icon")) ajax_action = "remove";
    else ajax_action = "add";
    jQuery.ajax({
        url: geodir_params.ajax_url,
        type: "GET",
        dataType: "json",
        ...
        success: function(data) {
            if (data.success)
                if (ajax_action == "remove") jQuery(".favorite_property_" + post_id + " a").removeClass("geodir-removetofav-icon").addClass("geodir-addtofav-icon").attr("title", geodir_params.text_add_fav).html('<i class="' + geodir_params.icon_fav + '"></i>' + " " + geodir_params.text_fav);
                else jQuery(".favorite_property_" + post_id + " a").removeClass("geodir-addtofav-icon").addClass("geodir-removetofav-icon").attr("title", geodir_params.text_remove_fav).html('<i class="' + geodir_params.icon_unfav + '"></i>' + " " + geodir_params.text_unfav);
            else alert(geodir_params.loading_listing_error_favorite)
        }
    });
    return false
}
...
function geodir_refresh_business_hour($this) {
    var d, $d, hours, day, mins, time, hasOpen = false,
        ...
        opens = [];
    d = new Date, utc = d.getTime() + d.getTimezoneOffset() * 60000, d = new Date(utc + parseInt(jQuery(".gd-bh-expand-range", $this).data("offsetsec")) * 1E3);
    ...
    if ($d.length) {
        ...
        } else $d.addClass("gd-bh-days-close");
...
function init_read_more() {
    ...
    jQuery(".geodir-category-list-view  .geodir-field-post_content").each(function() {
        jQuery(this).addClass("gd-read-more-wrap").wrapInner("<p></p>").append('<p class="gd-read-more"><a href="#" class="gd-read-more-button">' + geodir_params.txt_read_more + '<span class="gd-visuallyhidden">' + geodir_params.txt_about_listing + "</span></a></p>")
    });
    jQuery(".gd-read-more-wrap").each(function() {
        var height = jQuery(this).height();
        var maxHeight = parseInt(jQuery(this).css("max-height"), 10);
        if (height >= maxHeight) jQuery(this).find(".gd-read-more").show()
    });
...
jQuery(function($) {
    if (window.gdMaps == "osm" && $('input[name="street"]').length) geodir_osm_autocomplete_search()
});
    ...
    try {
        if (window.gdMaps == "osm" && jQuery('input[name="street"]').length) {
            ...
            jQuery('input[name="street"]', $form).autocomplete({
                ...
                if (!ul.hasClass("gd-osm-results")) ul.addClass("gd-osm-results");
                var label = item.display_name;
                if (label && this.term) label = gd_highlight(label, this.term);
...
function geodir_ajax_load_slider(slide) {
    if (real_srcset = jQuery(slide).find("img").attr("data-srcset")) jQuery(slide).find("img").attr("srcset", real_srcset);
    if (real_src = jQuery(slide).find("img").attr("data-src")) jQuery(slide).find("img").attr("src", real_src)
}
...
function geodir_init_slider($id) {
    ...
    jQuery("#" + $id + "_carousel").flexslider({
        ...
        directionNav: 1,
        prevText: '<i class="fas fa-angle-right"></i><span class="sr-only">' + geodir_params.txt_previous + "</span>",
        nextText: '<i class="fas fa-angle-right"></i><span class="sr-only">' + geodir_params.txt_next + "</span>",
        animationLoop: !0,
        ...
        start: function(slider) {
            ...
            if (real_srcset = jQuery(next).find("img").attr("data-srcset")) jQuery(next).find("img").attr("srcset", real_srcset);
            if (real_src = jQuery(next).find("img").attr("data-src")) jQuery(next).find("img").attr("src", real_src)
        },
        before: function(slider) {
            animatingTo = slider.slides.eq(slider.animatingTo);
            next_next = slider.slides.eq(slider.currentSlide + 2);
            geodir_ajax_load_slider(next_next);
            geodir_ajax_load_slider(animatingTo)
        },
        rtl: 1 == parseInt(geodir_params.is_rtl) ? !0 : !1
    })
}
...
function gd_init_rating_input() {
    jQuery(".gd-rating-input").each(function() {
        if (geodir_params.rating_type == "font-awesome") $type = "svg";
        else $type = "img";
        ...
        jQuery(this).find($type).each(function(index) {
            ...
            jQuery(this).hover(function() {
                ...
                if ($rating > $total) $rating = $rating - $total;
