jQuery( function ( $ ) {
	...
	var gdSystemStatus = {
		init: function() {
			$( document.body )
				.on( 'click', 'a.help_tip, a.gd-help-tip', this.preventTipTipClick )
		...
		generateReport: function() {
			...
			$( '.gd_status_table thead, .gd_status_table tbody' ).each( function() {
				if ( $( this ).is( 'thead' ) ) {
		...
		copyReport: function( evt ) {
			gdClearClipboard();
			gdSetClipboard( $( '#debug-report' ).find( 'textarea' ).val(), $( this ) );
			evt.preventDefault();
		},
	...
	$( '#log-viewer-select' ).on( 'click', 'h2 a.page-title-action', function( evt ) {
		...
		return window.confirm( geodir_admin_status_js_vars.delete_log_confirmation );
	});
});