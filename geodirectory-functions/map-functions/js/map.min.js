...
function create_marker(input,map_canvas_var){if(window.gdMaps=="osm")return create_marker_osm(input,map_canvas_var);if(map_canvas_var=="detail_page_map_canvas"){gd_single_marker_lat=input.lt;gd_single_marker_lon=input.ln}jQuery("#"+map_canvas_var).goMap();if(input.lt&&input.ln){var coord=new google.maps.LatLng(input.lt,input.ln);var marker_id=0;if(eval(map_canvas_var).enable_cat_filters)marker_id=input.mk_id;else marker_id=input.id;var title=geodir_htmlEscape(input.t);if(!input.i)input.i=geodir_all_js_msg.geodir_default_marker_icon;var cs=input.cs;var marker=jQuery.goMap.createMarker({id:marker_id,title:title,position:coord,visible:true,clickable:true,icon:input.i,label:cs});bounds.extend(coord);google.maps.event.addListener(marker,"spider_click",function(){is_zooming=true;jQuery("#"+map_canvas_var).goMap();var preview_query_str="";if(input.post_preview)preview_query_str="&post_preview="+input.post_preview;if(eval(map_canvas_var).bubble_size)var marker_url=eval(map_canvas_var).ajax_url+"&geodir_ajax=map_ajax&ajax_action=info&m_id="+
input.id+"&small=1"+preview_query_str;else var marker_url=eval(map_canvas_var).ajax_url+"&geodir_ajax=map_ajax&ajax_action=info&m_id="+input.id+preview_query_str;var loading='<div id="map_loading"></div>';gd_infowindow.open(jQuery.goMap.map,marker);gd_infowindow.setContent(loading);jQuery.ajax({type:"GET",url:marker_url,cache:false,dataType:"html",error:function(xhr,error){alert(error)},success:function(response){jQuery("#"+map_canvas_var).goMap();response=geodir_htmlEscape(response);gd_infowindow.setContent(response);
gd_infowindow.open(jQuery.goMap.map,marker);geodir_fix_marker_pos(map_canvas_var);setTimeout(function(){is_zooming=false},1E3)}});return});jQuery.goMap.oms.addMarker(marker);google.maps.event.addListener(marker,"visible_changed",function(){gd_infowindow.close(jQuery.goMap.map,marker)});return true}else return false}
...
"google"){directionsDisplay.setMap(jQuery.goMap.map);directionsDisplay.setPanel(document.getElementById(map_canvas+"_directionsPanel"));google.maps.event.addListener(directionsDisplay,"directions_changed",function(){computeTotalDistance(directionsDisplay.directions,map_canvas)});jQuery("#directions-options").show();var from_address=document.getElementById(map_canvas+"_fromAddress").value;var request={origin:from_address,destination:gd_single_marker_lat+","+gd_single_marker_lon,travelMode:gdGetTravelMode(),
unitSystem:gdGetTravelUnits()};directionsService.route(request,function(response,status){if(status==google.maps.DirectionsStatus.OK)directionsDisplay.setDirections(response);else alert(geodir_all_js_msg.address_not_found_on_map_msg+from_address)})}}
...
function initMapOSM(map_options){map_options=eval(map_options);map_options.zoom=parseInt(map_options.zoom);var options=map_options;var pscaleFactor;var pstartmin;var ajax_url=options.ajax_url;var token=options.token;var search_string=options.token;var mm=0;var maptype=options.maptype;var zoom=options.zoom;var latitude=options.latitude;var longitude=options.longitude;var maxZoom=options.maxZoom;var etype=options.etype;var autozoom=options.autozoom;var scrollwheel=options.scrollwheel;var streetview=
...
jQuery("#"+map_canvas+"_triggermap").closest(".geodir_map_container").toggleClass("geodir_map_container_fullscreen");window.setTimeout(function(){setGeodirMapSize(true);jQuery.goMap.map._onResize();jQuery.goMap.map.invalidateSize()},100)}jQuery.goMap.oms.addListener("spiderfy",function(markers){jQuery.goMap.map.closePopup()});window.oms=jQuery.goMap.oms}
