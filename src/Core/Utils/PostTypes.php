...
/**
 * A container for post type-related helper functions.
 *
 * @since 3.0.0
 */
final class PostTypes {
	 ...
	 */
	public static function get_posttypes($output = 'names') {
		$post_types = array();
//		$post_types = get_option('post_types');
		$post_types = geodirectory()->settings->get('post_types');
	 ...
	 */
	public static function get_ctp_slug($post_type){
		$post_types = self::get_posttypes('array');
	 ...
	 */
	public static function post_type_options($plural_name = false, $translated = false) {
		$post_types = self::get_posttypes('object');
	 ...
	 */
	public static function is_gd_post_type($post_type) {
		...
		if (!empty(self::$is_gd_post_type) && !empty(self::$is_gd_post_type[$post_type])) {
			return true;
		}

		$gd_posttypes = self::get_posttypes();

		if (!empty($gd_posttypes) && in_array($post_type, $gd_posttypes)) {
			self::$is_gd_post_type[$post_type] = true;
	...
	public static function post_type_object($post_type) {
		if (self::is_gd_post_type($post_type)) {
			$post_types = self::get_posttypes('object');
	 ...
	 */
	public static function post_type_name($post_type, $translated = false) {
		$post_type_obj = self::post_type_object($post_type);
	 ...
	 */
	public static function post_type_singular_name($post_type, $translated = false) {
		$post_type_obj = self::post_type_object($post_type);
	 ...
	 */
	public static function listing_owner_label($post_type, $translated = true) {
		$label = 'Listing Owner';
	 ...
	 * @return mixed|void
	 */
	public static function add_listing_allowed_post_types() {
	 ...
	 */
	public static function add_listing_default_post_type() {
		$post_types = self::add_listing_allowed_post_types();
	 ...
	 */
	public static function search_default_post_type() {
		$post_types = self::get_posttypes();
	 ...
	 * @return bool $return.
	 */
	public static function add_listing_check_post_type($post_type) {
		if (!self::is_gd_post_type($post_type)) {
			return false;
		}

		$allowed_post_types = self::add_listing_allowed_post_types();
	 ...
	 * @return array
	 */
	public static function fav_allowed_post_types(){
		$postypes = self::get_posttypes('array');
	 ...
	 */
	public static function posttype_link($link, $post_type) {
		if (in_array($post_type, self::get_posttypes())) {
	 ...
	 */
	public static function get_post_type_singular_label($post_type, $echo = false, $translate = false) {
	 ...
	 * @return void|string Label.
	 */
	public static function get_post_type_plural_label($post_type, $echo = false, $translate = false) {
		$all_postypes = self::get_posttypes();
	 ...
	 */
	public static function custom_update_messages($messages) {
		$post = $GLOBALS['post'];
	 ...
	 */
	public static function get_posttype_info($post_type = '') {
		$post_types = self::get_posttypes('array');
		$post_types = stripslashes_deep($post_types);
	 ...
	 */
	public static function get_default_posttype() {
		$post_types = apply_filters('geodir_get_default_posttype', self::get_posttypes('object'));

		$stype = false;

		foreach ($post_types as $post_type => $info) {
			if (self::cpt_has_post($post_type)) {
				$stype = $post_type;
	 ...
	 */
	public static function cpt_has_post($post_type) {
		$has_post = false;

		// Check global cached.
		if (! (is_array(self::$cpt_has_post) && isset(self::$cpt_has_post[$post_type]))) {
			self::cpt_set_has_post();
		}
	 ...
	 * @since 2.3.85
	 */
	public static function cpt_set_has_post() {
		global $wpdb;
	...
	public static function get_current_posttype() {
		...
		if (empty($geodir_post_type) && geodir_is_page('search')) {
			$geodir_post_type = self::get_default_posttype();
		}
	 ...
	 */
	public static function get_posts_default_sort($post_type) {
		global $wpdb;
	 ...
	 */
	public static function get_sort_options($post_type) {
		global $wpdb;
	 ...
	 */
	public static function display_sort_options($post_type, $args = array()) {
		global $wp_query;
		...
		$sort_options_raw = self::get_sort_options($post_type);
	...
	public static function reorder_post_types() {
	 ...
	 */
	public static function cpt_permalink_rewrite_slug($post_type, $post_type_obj = NULL) {
	 ...
	 * @return void.
	 */
	public static function check_column_search_title($post_type) {
	 ...
	 */
	public static function generate_title_keywords($force = false) {
		$post_types = self::get_posttypes();
		...
		if (!empty($post_types)) {
			foreach ($post_types as $post_type) {
				$generated += (int)self::cpt_generate_title_keywords($post_type, $force);
			}
		}
	 ...
	 * @return int No. of keywords generated.
	 */
	public static function cpt_generate_title_keywords($post_type, $force = false) {
		global $wpdb;

		// Check & add column _search_title.
		self::check_column_search_title($post_type);
	 ...
	 * @since 2.3.39
	 */
	public static function cpt_requires_address($post_type) {
	 ...
	 */
	public static function cpt_rewrite_slug_options() {
		$post_types = self::get_posttypes('array');
		$options = array();
...
add_filter('post_updated_messages', 'AyeCode\GeoDirectory\Core\Utils\PostTypes::custom_update_messages');
