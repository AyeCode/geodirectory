...
/**
 * A container for location-related helper functions.
 *
 * @since 3.0.0
 */
final class Locations {
	 ...
	 */
	public static function create_location_slug($location_string) {
		global $geodirectory;
	 ...
	 * @param bool $return_array Optional. Default false.
	 */
	public static function get_country_dl($post_country = '', $prefix = '', $return_array = false) {
		global $wpdb;

		$rows = self::wp_countries();
	 ...
	 */
	public static function wp_countries($args = array(), $split = true) {
		$countries = wp_country_database()->get_countries($args);

		// Split UK.
		if ($split && self::split_uk() && !empty($countries)) {
			$_countries = array();
			$uk = array();
			$country_id = 0;
	 ...
	 */
	public static function get_address_by_lat_lan($lat, $lng) {
		$url = 'https://maps.googleapis.com/maps/api/geocode/json?latlng=' . trim($lat) . ',' . trim($lng) . \AyeCode\GeoDirectory\Core\Utils\Maps::google_geocode_api_key(true);

		// maybe add language.
		$lang = \AyeCode\GeoDirectory\Core\Utils\Maps::map_language();
		if ($lang && 'en' !== $lang) {
	 ...
	 */
	public static function get_current_location_terms($location_array_from = null, $gd_post_type = '') {
		...
		if (self::core_multi_city() && !geodir_is_page('single') && !geodir_is_page('search')) {
			$location_array = array();
		}
	...
	public static function location_name_from_slug($slug, $type) {
		// Function content from old file
	}
	 ...
	 * @return bool|string
	 */
	public static function get_location_link($which_location = 'current') {
		$location_link = trailingslashit(get_permalink(geodir_location_page_id()));

		if ($which_location == 'base') {
			return $location_link;
		} else {
			$location_terms = self::get_current_location_terms();
	 ...
	 */
	public static function get_osm_address_by_lat_lan($lat, $lng) {
	 ...
	 */
	public static function replace_location_variables($content, $location_array = array(), $sep = NULL, $gd_page = '') {
		if (empty($content)) {
			return $content;
		}

		$location_replace_vars = self::location_replace_vars($location_array, $sep, $gd_page);
	 ...
	 */
	public static function location_replace_vars($location_array = array(), $sep = NULL, $gd_page = '') {
		global $wp, $gd_post;

		// Private address
		$check_address = ($gd_page == 'single' || geodir_is_page('single')) && !empty($gd_post) && \AyeCode\GeoDirectory\Core\Utils\PostTypes::supports($gd_post->post_type, 'private_address') ? true : false;

		if (empty($location_array)) {
			$location_array = self::get_current_location_terms('query_vars');
		}

		if (class_exists('GeoDir_Location_City') && \AyeCode\GeoDirectory\Core\Utils\Settings::get('lm_url_filter_archives_on_single', 'city') != 'city' && !( !empty($location_array) && !empty($location_array['country']) && !empty($location_array['region']) && !empty($location_array['city'])) && !empty($gd_post) && ( $gd_page == 'single' || geodir_is_page('single')) && \AyeCode\GeoDirectory\Core\Utils\PostTypes::supports($gd_post->post_type, 'location') && !empty($gd_post->country) && !empty($gd_post->region) && !empty($gd_post->city)) {
		...
		foreach ($location_terms as $type => $location) {
			...
			if ($check_address && !empty($location)) {
				$location = self::post_address($location, $location_type, $gd_post, '');
			}
		...
		if (!empty($location_array)) {
			...
			foreach ($location_array as $type => $location) {
				...
				if ($check_address && !empty($location_name)) {
					$location_name = self::post_address($location_name, $location_type, $gd_post, '');
				}
	 ...
	 */
	public static function location_slug_check($slug) {
	 ...
	 * @return mixed|void
	 */
	public static function geo_by_ip($ip = '') {
		$geo = array();

		$data = self::ip_api_data($ip);
	 ...
	 */
	public static function geoplugin_data($ip = '') {
		global $wp_version;
	 ...
	 */
	public static function ip_api_data($ip = '') {
		global $wp_version;
	 ...
	 */
	public static function get_timezone_by_lat_lon($latitude, $longitude, $timestamp = 0) {
		global $wp_version;
		...
		if (!empty($latitude) && !empty($longitude)) {
			...
			$api_url .= '?key=' . \AyeCode\GeoDirectory\Core\Utils\Maps::google_geocode_api_key();
	 ...
	 */
	public static function get_gps_from_address($address = array(), $wp_error = false) {
		$api = \AyeCode\GeoDirectory\Core\Utils\Maps::active_map();

		$api = apply_filters('geodir_post_gps_from_address_api', $api);

		if ($api == 'google' || $api == 'auto') {
			$_gps = self::google_get_gps_from_address($address, $wp_error);
		} elseif ($api == 'osm') {
			$_gps = self::osm_get_gps_from_address($address, $wp_error);
		} else {
		...
		if (is_array($_gps) && !empty($_gps['latitude']) && !empty($_gps['longitude'])) {
			...
			if ($wp_error) {
				if (is_wp_error($_gps)) {
					...
					return new WP_Error('geodir-gps-from-address', esc_attr(__('Failed to retrieve GPS data from a address using API.', 'geodirectory')));
				}
			} else {
				return NULL;
			}
	 ...
	 */
	public static function google_get_gps_from_address($address, $wp_error = false) {
		global $wp_version;
		...
		$google_api_key = \AyeCode\GeoDirectory\Core\Utils\Maps::google_geocode_api_key();
		if ($google_api_key) {
	 ...
	 * @return bool|WP_Error GPS data or WP_Error on failure.
	 */
	public static function osm_get_gps_from_address($address, $wp_error = false) {
		global $wp_version;
		...
		if (!empty($data) && is_array($data)) {
			...
			if (is_array($address) && !empty($address['city']) && !empty($address['zip'])) {
				...
				return self::osm_get_gps_from_address($address, $wp_error);
			}
	 ...
	 * @return bool|WP_Error GPS data or WP_Error on failure.
	 */
	public static function core_multi_city() {
		return (!defined('GEODIRLOCATION_VERSION') && \AyeCode\GeoDirectory\Core\Utils\Settings::get('multi_city'));
	}
	 ...
	 */
	public static function post_address($value, $key, $post, $default = NULL) {
		if (geodir_is_block_demo()) {
			return $value;
		}

		if (!empty($post) && !\AyeCode\GeoDirectory\Core\Utils\User::can_see_private_address($post)) {
			switch ($key) {
				case 'address':
				case 'street':
				case 'street2':
	 ...
	 */
	public static function split_uk() {
		$split_uk = \AyeCode\GeoDirectory\Core\Utils\Settings::get('split_uk') ? true : false;
...
add_filter('geodir_replace_location_variables', 'AyeCode\GeoDirectory\Core\Utils\Location::replace_location_variables');
add_filter('geodir_location_slug_check', 'AyeCode\GeoDirectory\Core\Utils\Location::location_slug_check');
