...
/**
 * A service for image-related helper functions.
 *
 * @since 3.0.0
 */
final class Image {
	 ...
	 * @var array
	 */
	private $image_dimension = [];

	/**
	 * Constructor.
	 *
	 * Registers the filter for external srcset handling.
	 */
	public function __construct() {
		add_filter( 'wp_calculate_image_srcset', [ $this, 'set_external_srcset' ], 10, 5 );
	}

	/**
	 * Replaces image tag src and class values for lazy loading.
	 *
	 ...
	 */
	public function image_tag_ajaxify( $img_tag, $lazy_load = true ) {
		$strip   = [ "src=", "srcset=" ];
		$replace = [ "src='data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAGQAAAABCAQAAACC0sM2AAAADklEQVR42mP8X88wLAAAK5IBgMYCdqgAAAAASUVORK5CYII=' data-src=", "data-srcset=" ];
		if ( $lazy_load ) {
			$strip[]   = 'class="';
	...
	/**
	 * Gets an image src path from a GeoDirectory image object.
	 *
	 * @since 2.0.0
	 *
	 ...
	 */
	public function get_image_src( $image, $size = 'medium' ) {
		$img_src = '';
	 ...
	 */
	public function get_image_tag( $image, $size = 'medium', $align = '', $classes = '' ) {
		$meta    = isset( $image->metadata ) ? maybe_unserialize( $image->metadata ) : '';
	 ...
	 */
	public function get_screenshot( $url, $params = [] ) {
	...
	/**
	 * Gets a screenshot for a URL stored in a post field.
	 *
	 * @param string $field    The post meta field key.
	 * @param array  $sizes    Screenshot size parameters.
	 * @param array  $the_post The post object/array.
	 * @return string The screenshot URL, or empty string.
	 */
	public function get_field_screenshot( $field, $sizes = [], $the_post = [] ) {
		$gd_post = null; // Assuming we need to get the global here if not passed in.
		if ( empty( $the_post ) && class_exists( 'GeoDirectory' ) ) {
			$gd_post = $GLOBALS['gd_post'];
		}

		if ( empty( $sizes ) || ! is_array( $sizes ) ) {
			$sizes = [ 'w' => 825, 'h' => 430, 'image' => 1 ];
		}
	 ...
	 *
	 * @param string $field_raw The raw URL string.
	 * @return mixed|void
	 */
	public function get_video_screenshot( $field_raw ) {
		$screenshot          = '';
	 ...
	 */
	public function get_images( $post_id = 0, $limit = '', $logo = false, $revision_id = '', $types = [], $fallback_types = [ 'logo', 'cat_default', 'cpt_default', 'listing_default', 'website_screenshot' ], $status = '' ) {
		$gd_post = null; // Assuming we need to get the global here if not passed in.
		...
		$the_post = ! empty( $gd_post ) && $gd_post->ID == $post_id ? $gd_post : [];
							...
							if ( isset( $the_post->{$field} ) && esc_url( $the_post->{$field} ) ) {
								...
								if ( ! empty( $attachments ) ) {
									foreach ( $attachments as $ia => $attachment ) {
										$media_url   = $this->get_image_src( $attachment );
										$media_image = $this->get_screenshot( $media_url, [ 'w' => 825, 'h' => 430, 'image' => 1 ] );
										...
										if ( $media_image ) {
											...
											$image->user_id       = 0;
											$image->title         = ! empty( $attachment->title ) ? $attachment->title : wp_sprintf( __( 'Screenshot for %s', 'geodirectory' ), esc_attr( $field ) );
											...
											$image->featured      = 0;
											$image->is_approved   = 1;
											$image->metadata      = [ 'media_url' => $media_url, 'mime_type' => $attachment->mime_type ];
											$image->type          = esc_attr( $field ) . '_screenshot';
											$new_images[]         = $image;
										}
									}
								} else {
									$image_src = $this->get_field_screenshot( $field, [], $the_post );
									...
									if ( $image_src ) {
										...
										$image->post_id     = $post_id;
										$image->user_id     = 0;
										$image->title       = wp_sprintf( __( 'Screenshot for %s', 'geodirectory' ), esc_attr( $field ) );
										$image->caption     = '';
				...
				if ( $fallback_type == 'cat_default' ) {
					...
					if ( ! empty( $term_img ) ) {
						...
						if ( ! empty( $default_img_id ) && ! empty( $term_img['src'] ) ) {
							...
							$post_images[]       = $image;
							break;
						} elseif ( ! empty( $default_img_id ) && absint( $default_img_id ) > 0 ) {
	...
	/**
	 * Check if a string is a Font Awesome icon class.
	 *
	 * @param string $icon Font awesome icon.
	 * @return bool True if font awesome icon.
	 */
	public function is_fa_icon( $icon ) {
		$return = false;
		if ( $icon != '' ) {
			$fa_icon = trim( $icon );
	...
	/**
	 * Check if a string is an icon URL.
	 *
	 * @param string $icon Icon url.
	 * @return bool True if icon url.
	 */
	public function is_icon_url( $icon ) {
		$return = false;
		if ( $icon != '' ) {
			$icon = trim( $icon );
	 ...
	 */
	public function post_has_image_types( $types = [], $post_id = 0, $revision_id = 0 ) {
		if ( ! empty( $types ) ) {
			if ( is_scalar( $types ) ) {
			...
			if ( ! empty( $image_types ) ) {
				...
				foreach ( $image_types as $type ) {
					if ( in_array( $type, [ 'post_images', 'comment_images' ] ) ) {
						$valid_types[] = $type;
	 ...
	 */
	public function get_image_dimension( $image_url, $default = [] ) {
		if ( ! empty( $this->image_dimension[ $image_url ] ) ) {
			return $this->image_dimension[ $image_url ];
		}

		if ( empty( $image_url ) ) {
			$this->image_dimension[ $image_url ] = $default;
		...
		if ( strpos( $image_url, 'http://' ) !== false || strpos( $image_url, 'https://' ) !== false ) {
			$this->image_dimension[ $_image_url ] = $default;
		...
		if ( is_file( $image_url ) && file_exists( $image_url ) ) {
			...
			if ( ! empty( $size ) && ! empty( $size[0] ) && ! empty( $size[1] ) ) {
				$dimension = [ 'width' => $size[0], 'height' => $size[1] ];
			}
		}

		$dimension = ! empty( $dimension ) ? $dimension : $default;

		$this->image_dimension[ $_image_url ] = $dimension;
	 ...
	 */
	public function getimagesize( $image_path ) {
		...
		if ( empty( $size ) ) {
			...
			if ( preg_match( '/\.svg$/i', $image_path ) && ( $xml = @simplexml_load_file( $image_path ) ) !== false ) {
				...
				if ( ! empty( $attributes ) && ( isset( $attributes->viewBox ) || isset( $attributes->viewbox ) ) ) {
					...
					$viewbox = $attributes->viewBox ? explode( ' ', (string) $attributes->viewBox ) : explode( ' ', (string) $attributes->viewbox );

					$size[0] = isset( $attributes->width ) && preg_match( '/\d+/', (string) $attributes->width, $value ) ? (int) $value[0] : ( count( $viewbox ) == 4 ? (int) trim( $viewbox[2] ) : 0 );
					$size[1] = isset( $attributes->height ) && preg_match( '/\d+/', (string) $attributes->height, $value ) ? (int) $value[0] : ( count( $viewbox ) == 4 ? (int) trim( $viewbox[3] ) : 0 );
				}
			}
		}
	 ...
	 * @return mixed
	 */
	public function set_external_srcset( $sources, $size_array, $image_src, $image_meta, $attachment_id ) {
