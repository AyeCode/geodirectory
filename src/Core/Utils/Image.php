<?php
/**
 * GeoDirectory Image Utility Class
 *
...
/**
 * A container for image-related helper functions.
 *
 * @since 3.0.0
 */
final class Image {
	 ...
	 * @var array
	 */
	private static $geodir_image_dimension = [];
	 ...
	 */
	public static function geodir_image_tag_ajaxify($img_tag, $lazy_load = true) {
	 ...
	 */
	public static function geodir_get_image_src($image, $size = 'medium') {
		$img_src = '';
	 ...
	 */
	public static function geodir_get_image_tag($image, $size = 'medium', $align = '', $classes = '') {
		$meta = isset($image->metadata) ? maybe_unserialize($image->metadata) : '';
		$img_src = self::geodir_get_image_src($image, $size);
		$width = '';
		$height = '';
		if ($size) {
	 ...
	 */
	public static function geodir_get_screenshot($url, $params = array()) {
		...
		if (!empty($params['image']) && (empty($params['vpw']) || empty($params['vph']))) {
			...
			$dimension = self::geodir_get_image_dimension($url);
	...
	public static function geodir_get_field_screenshot($field, $sizes = array(), $the_post = array()) {
		...
		if (isset($the_post->{$field}) && esc_url($the_post->{$field})) {
			...
			if ($video = self::geodir_get_video_screenshot($the_post->{$field})) {
				$url = $video;
			} else {
				$url = self::geodir_get_screenshot($the_post->{$field}, $sizes);
			}
		}
	 ...
	 * @return mixed|void
	 */
	public static function geodir_get_video_screenshot($field_raw) {
		$screenshot = '';
	 ...
	 */
	public static function geodir_get_images($post_id = 0, $limit = '', $logo = false, $revision_id = '', $types = array(), $fallback_types = array('logo', 'cat_default', 'cpt_default', 'listing_default', 'website_screenshot'), $status = '') {
		$gd_post = null; // Assuming we need to get the global here if not passed in.
							...
							if (isset($the_post->{$field}) && esc_url($the_post->{$field})) {
								...
								if (!empty($attachments)) {
									foreach ($attachments as $ia => $attachment) {
										$media_url = self::geodir_get_image_src($attachment);
										$media_image = self::geodir_get_screenshot($media_url, array('w' => 825, 'h' => 430, 'image' => 1));
									...
									$image_src = self::geodir_get_field_screenshot($field, array(), $the_post);
	 ...
	 */
	public static function geodir_is_fa_icon($icon) {
		$return = false;
		if ($icon != '') {
			$fa_icon = trim($icon);
	 ...
	 */
	public static function geodir_is_icon_url($icon) {
		$return = false;
		if ($icon != '') {
			$icon = trim($icon);
	 ...
	 */
	public static function geodir_post_has_image_types($types = array(), $post_id = 0, $revision_id = 0) {
		if (!empty($types)) {
			if (is_scalar($types)) {
	 ...
	 */
	public static function geodir_get_image_dimension($image_url, $default = array()) {
		if (!empty(self::$geodir_image_dimension[$image_url])) {
			return self::$geodir_image_dimension[$image_url];
		}

		if (empty($image_url)) {
			self::$geodir_image_dimension[$image_url] = $default;
		...
		if (strpos($image_url, 'http://') !== false || strpos($image_url, 'https://') !== false) {
			self::$geodir_image_dimension[$_image_url] = $default;
		...
		if (is_file($image_url) && file_exists($image_url)) {
			$size = self::geodir_getimagesize(trim($image_url));
		...
		self::$geodir_image_dimension[$_image_url] = $dimension;
	 ...
	 */
	public static function geodir_getimagesize($image_path) {
	 ...
	 * @return mixed
	 */
	public static function geodir_set_external_srcset($sources, $size_array, $image_src, $image_meta, $attachment_id) {
...
add_filter('wp_calculate_image_srcset', 'AyeCode\GeoDirectory\Core\Utils\Image::geodir_set_external_srcset', 10, 5);
